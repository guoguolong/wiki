...
在上面的介绍中，提到了 Unicode 是一本很厚的字典，她将全世界所有的字符定义在一个集合里。这么多的字符不是一次性定义的，而是分区定义。每个区可以存放 65536 个（2^16）字符，称为一个平面（plane）。目前，一共有 17 个平面，也就是说，整个 Unicode 字符集的大小现在是 2^21。

***基本平面***:  最前面的 65536 个字符位，称为基本平面（简称 BMP ），它的码点范围是从 0 到 2^16-1，写成 16 进制就是从 U+0000 到 U+FFFF。所有最常见的字符都放在这个平面，这是 Unicode 最先定义和公布的一个平面。

***辅助平面***:  剩下的字符都放在辅助平面（简称 SMP ），码点范围从 U+010000 到 U+10FFFF。
...
辅助平面的字符位共有 2^20 个，因此表示这些字符至少需要 20 个二进制位。我们将码点用U来表示，U > 0xFFFF。然后计算 U' = U - 0x10000，将这 U' 这20 个二进制位分成两半（前面不足补0）：

* 前 10 位映射在 U+D800 到 U+DBFF，称为高位（H）

* 后 10 位映射在 U+DC00 到 U+DFFF，称为低位（L）

也就是说，一个辅助平面的字符，被拆成两个基本平面的字符表示：

|字节位置|格式|范围|备注|
|---|---|---|---|
|前2字节| 1101 10xx xxxx xxxx |U+D800 到 U+DBFF | xxx为U'的前10位 |
|后2字节| 1101 11yy yyyy yyyy |U+DC00 到 U+DFFF | yyy为U'的后10位 |

因此，当我们遇到两个字节，发现它的码点在 U+D800 到 U+DBFF 之间，就可以断定，紧跟在后面的两个字节的码点，应该在 U+DC00 到 U+DFFF 之间，这四个字节必须放在一起解读。

思考：为什么 U 要 换算为 U'呢？

> 辅助平面U的最大值为0x10FFFF，21位（超过20位），但是4字节编码 xxxx、yyy不够填充啊， 因此需要减去0x10000得到U'，此时U'的最大值是0xFFFFF，所以肯定可以用20个二进制位表示。然后程序读取的时候再把这个 0x10000 加回来呗。

### 汉字"𠮷"的例子

接下来，以汉字"𠮷"为例，说明 UTF-16 编码方式是如何工作的。

汉字"𠮷"的 Unicode 码点为 0x20BB7，该码点显然超出了基本平面的范围（0x0000 - 0xFFFF），因此需要使用四个字节表示。

1. 首先用 0x20BB7 减去 0x10000 计算出超出的部分，然后将其用 20 个二进制位表示（不足前面补 0 ），结果：

```c
中间码 = 0x20BB7 - 0x10000 = 0x10BB7 = 0001 0000 1011 1011 0111
```

2. 接着，将前 10 位映射到 U+D800 到 U+DBFF 之间，U+D800 对应的二进制数为 1101100000000000，直接填充***中间码***的 前10 个二进制位即可，得到：

>1101 10***00 0100 0010*** = 0xD842

3. 最后，将后 10 位映射到 U+DC00 到 U+DFFF 之间，U+DC00 对应的二进制数为 1101100000000000，直接填充***中间码***后 10 个二进制位即可，得到：

>1101 11***11 1011 0111*** = 0xDFB7

因此得出汉字"𠮷"的 UTF-16 编码为: 
...
```
推荐一个网站换算Unicode码点和编码的转换：https://www.qqxiuzi.cn/bianma/Unicode-UTF.php